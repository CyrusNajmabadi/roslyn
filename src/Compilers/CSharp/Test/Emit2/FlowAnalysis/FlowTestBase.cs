...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests
{
    public abstract class FlowTestBase : SemanticModelTestBase
    {
        ...
        {
            var flowDiagnostics = CSharpBindingDiagnosticBag.GetInstance();
            foreach (var method in AllMethods(compilation.SourceModule.GlobalNamespace))
            {
                ...
                var boundBody = MethodCompiler.BindSynthesizedMethodBody(sourceSymbol, compilationState, new CSharpBindingDiagnosticBag(new DiagnosticBag()));
                if (boundBody != null)
                {
