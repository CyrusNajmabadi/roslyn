...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal partial class Binder
    {
        ...
        internal BoundExpression BindQuery(QueryExpressionSyntax node, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private (QueryTranslationState, RangeVariableSymbol) MakeInitialQueryTranslationState(QueryExpressionSyntax node, CSharpBindingDiagnosticBag diagnostics)
        {
            var fromClause = node.FromClause;
        ...
        private RangeVariableSymbol PrepareQueryTranslationStateForContinuation(QueryTranslationState state, QueryContinuationSyntax continuation, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private BoundExpression BindQueryInternal1(QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private BoundExpression BindQueryInternal2(QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
            ...
            while (true)
            {
                if (state.clauses.IsEmpty())
                {
                    ...
                    if (IsDegenerateQuery(state))
                    {
                        ...
                        BoundExpression? unoptimized = FinalTranslation(state, CSharpBindingDiagnosticBag.Discarded);
        ...
        private BoundExpression FinalTranslation(QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
            Debug.Assert(state.clauses.IsEmpty());
                ...
                case SyntaxKind.GroupClause:
                    {
                        ...
                        var d = CSharpBindingDiagnosticBag.GetInstance(diagnostics);
        ...
        private void ReduceQuery(QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private void ReduceWhere(WhereClauseSyntax where, QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private void ReduceJoin(JoinClauseSyntax join, QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private void ReduceOrderBy(OrderByClauseSyntax orderby, QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private void ReduceFrom(FromClauseSyntax from, QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
            var x1 = state.rangeVariable;
        ...
        private UnboundLambda MakePairLambda(CSharpSyntaxNode node, QueryTranslationState state, RangeVariableSymbol x1, RangeVariableSymbol x2, bool withDependencies)
        {
            ...
            LambdaBodyFactory bodyFactory = (LambdaSymbol lambdaSymbol, Binder lambdaBodyBinder, CSharpBindingDiagnosticBag d) =>
            {
        ...
        private void ReduceLet(LetClauseSyntax let, QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private BoundBlock CreateLambdaBlockForQueryClause(ExpressionSyntax expression, BoundExpression result, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private BoundExpression MakePair(CSharpSyntaxNode node, string field1Name, BoundExpression field1Value, string field2Name, BoundExpression field2Value, QueryTranslationState state, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        {
            return MakeQueryUnboundLambda(expression, new QueryUnboundLambdaState(this, qvm, parameters, (LambdaSymbol lambdaSymbol, Binder lambdaBodyBinder, CSharpBindingDiagnosticBag diagnostics) =>
            {
        ...
        {
            return MakeQueryUnboundLambda(expression, new QueryUnboundLambdaState(this, qvm, ImmutableArray.Create(parameter), (LambdaSymbol lambdaSymbol, Binder lambdaBodyBinder, CSharpBindingDiagnosticBag diagnostics) =>
            {
                lambdaBodyBinder = lambdaBodyBinder.GetRequiredBinder(expression);
                BoundExpression boundExpression = lambdaBodyBinder.BindValue(expression, diagnostics, BindValueKind.RValue);
        ...
        protected BoundCall MakeQueryInvocation(CSharpSyntaxNode node, BoundExpression receiver, string methodName, BoundExpression arg, CSharpBindingDiagnosticBag diagnostics
#if DEBUG
        ...
        protected BoundCall MakeQueryInvocation(CSharpSyntaxNode node, BoundExpression receiver, string methodName, ImmutableArray<BoundExpression> args, CSharpBindingDiagnosticBag diagnostics
#if DEBUG
            , string? expectedMethodName
#endif
            )
        ...
        protected BoundCall MakeQueryInvocation(CSharpSyntaxNode node, BoundExpression receiver, string methodName, TypeSyntax typeArgSyntax, TypeWithAnnotations typeArg, CSharpBindingDiagnosticBag diagnostics
#if DEBUG
            , string? expectedMethodName
#endif
            )
        ...
        protected BoundCall MakeQueryInvocation(CSharpSyntaxNode node, BoundExpression receiver, string methodName, SeparatedSyntaxList<TypeSyntax> typeArgsSyntax, ImmutableArray<TypeWithAnnotations> typeArgs, ImmutableArray<BoundExpression> args, CSharpBindingDiagnosticBag diagnostics
#if DEBUG
            , string? expectedMethodName
#endif
            )
        ...
        protected BoundExpression MakeConstruction(CSharpSyntaxNode node, NamedTypeSymbol toCreate, ImmutableArray<BoundExpression> args, CSharpBindingDiagnosticBag diagnostics)
        {
