...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        internal BoundExpression BindDeconstruction(AssignmentExpressionSyntax node, CSharpBindingDiagnosticBag diagnostics, bool resultIsUsedOverride = false)
        {
            var left = node.Left;
            ...
            BoundDeconstructValuePlaceholder? rightPlaceholder = null)
        {
            ...
            var deconstructionDiagnostics = BindingDiagnosticBag.CreateNewInstance(new DiagnosticBag(), diagnostics.DependenciesBag);
        ...
        private BoundExpression FixTupleLiteral(ArrayBuilder<DeconstructionVariable> checkedVariables, BoundExpression boundRHS, CSharpSyntaxNode syntax, CSharpBindingDiagnosticBag diagnostics)
        {
            Debug.Assert(diagnostics.DiagnosticBag is object);
...
#pragma warning restore format
        ...
        /// </summary>
        private void SetInferredTypes(ArrayBuilder<DeconstructionVariable> variables, ImmutableArray<TypeSymbol> foundTypes, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        private BoundExpression SetInferredType(BoundExpression expression, TypeSymbol type, CSharpBindingDiagnosticBag diagnostics)
        {
        ...
        /// </summary>
        private void FailRemainingInferences(ArrayBuilder<DeconstructionVariable> variables, CSharpBindingDiagnosticBag diagnostics)
        {
            int count = variables.Count;
            for (int i = 0; i < count; i++)
            {
        ...
        /// </summary>
        private TypeSymbol? MakeMergedTupleType(ArrayBuilder<DeconstructionVariable> lhsVariables, BoundTupleLiteral rhsLiteral, CSharpSyntaxNode syntax, CSharpBindingDiagnosticBag? diagnostics)
        {
        ...
        private BoundTupleExpression DeconstructionVariablesAsTuple(CSharpSyntaxNode syntax, ArrayBuilder<DeconstructionVariable> variables,
            CSharpBindingDiagnosticBag diagnostics, bool ignoreDiagnosticsFromTuple)
        {
            int count = variables.Count;
        ...
        private BoundBadExpression MissingDeconstruct(BoundExpression receiver, SyntaxNode rightSyntax, int numParameters, CSharpBindingDiagnosticBag diagnostics,
