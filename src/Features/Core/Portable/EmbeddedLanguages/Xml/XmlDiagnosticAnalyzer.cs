...
namespace Microsoft.CodeAnalysis.Features.EmbeddedLanguages.Xml
{
    ...
    /// </summary>
    internal sealed class XmlDiagnosticAnalyzer : AbstractBuiltInCodeStyleDiagnosticAnalyzer
    {
        public const string DiagnosticId = "XML01";

        private readonly EmbeddedLanguageInfo _info;

        public XmlDiagnosticAnalyzer(EmbeddedLanguageInfo info)
            : base(DiagnosticId,
                   new LocalizableResourceString(nameof(WorkspacesResources.Xml_issue_0), WorkspacesResources.ResourceManager, typeof(WorkspacesResources)),
                   new LocalizableResourceString(nameof(WorkspacesResources.Xml_issue_0), WorkspacesResources.ResourceManager, typeof(WorkspacesResources)))
        {
            _info = info;
        }
        ...
        public void Analyze(SemanticModelAnalysisContext context)
        {
            ...
            var option = optionSet.GetOption(XmlFeatureOptions.ReportInvalidXmlPatterns, syntaxTree.Options.Language);
            if (!option)
            {
                return;
            }

            if (!(options is WorkspaceAnalyzerOptions workspaceOptions))
            {
                return;
            }

            var workspace = workspaceOptions.Solution.Workspace;
        ...
        private void AnalyzeToken(
            SemanticModelAnalysisContext context, XmlPatternDetector detector, 
